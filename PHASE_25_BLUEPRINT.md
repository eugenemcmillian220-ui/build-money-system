...
## Architecture
- **Semantic Vector Memory**: Integrates `pgvector` with Hugging Face embeddings to allow agents to recall past generations based on semantic similarity rather than just keyword matches.
...
2. **Embedding Integration** (`src/lib/openai.ts` compatibility layer for embeddings)
