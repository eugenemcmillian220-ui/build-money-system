...
async function callProvider(provider: (typeof CHAIN)[number], input: Input): Promise<string> {
  ...
  const model = input.model ?? (
    provider === 'opencodeGo' ? (process.env.OPENCODE_GO_MODEL ?? 'minimax-m2.5') :
    provider === 'opencodeZen' ? (process.env.OPENCODE_ZEN_MODEL ?? 'kimi-k2-0711-preview:free') : 'gpt-4o-mini'
  );
  ...
  const body = anthropic
    ? { model, max_tokens: MAX_TOKENS, messages: [{ role: 'user', content: input.systemPrompt + '\n\n' + input.userPrompt }] }
    : { model, messages: [{ role: 'system', content: input.systemPrompt }, { role: 'user', content: input.userPrompt }], max_tokens: MAX_TOKENS };
  ...
  if (!text || text.trim().length === 0) throw new Error(provider + '/' + model + ' returned empty response');
  return text;
}
...
export async function callLLM(input: Input): Promise<string> {
  ...
  for (const p of CHAIN) {
    try { return await withRetry(() => callProvider(p, input), 2, 500); }
    catch (e) { console.warn('[llm-router] ' + p + ' failed:', e instanceof Error ? e.message : e); last = e; }
  }
