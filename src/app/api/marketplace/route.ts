...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const listings = await marketplace.getListings({ category, sellerId, maxPrice, minRating });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === "purchase") {
      ...
      const purchase = await marketplace.purchaseListing(listingId, buyerId);
    ...
    const listing = await marketplace.addListing({ title, description, category, price, sellerId, tags });
