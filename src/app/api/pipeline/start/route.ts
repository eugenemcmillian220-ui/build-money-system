...
export async function POST(req: NextRequest) {
  ...
  if (!parsed.success) return fail(ErrorCode.INVALID_INPUT, 'Invalid pipeline start payload', 400, parsed.error.flatten())
  ...
  if (!railwayUrl || !railwaySecret) {
    ...
    return fail(ErrorCode.INTERNAL_ERROR, 'Pipeline service not configured', 503)
  }

  const creditResult = await checkAndDeductCredits(user.id, PIPELINE_COST_CREDITS)
  if (!creditResult.success) {
    return fail(ErrorCode.FORBIDDEN, creditResult.error ?? 'Insufficient credits', 402, { remainingCredits: 0 })
  }
  ...
  if (jobError || !job) {
    ...
    return fail(ErrorCode.INTERNAL_ERROR, 'Failed to create job', 500)
  }
  ...
  try {
    ...
    return fail(ErrorCode.INTERNAL_ERROR, 'Pipeline service unavailable. Credits refunded.', 503, { jobId: job.id })
  }
