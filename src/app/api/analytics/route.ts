...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const metrics = await analyticsEngine.getMetrics({ name, category, userId, since, until });

    const from = since ? new Date(since) : new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
    const to = until ? new Date(until) : new Date();
    const report = await analyticsEngine.generateReport({ from, to });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const metric = await analyticsEngine.trackMetric({ name, value, unit, category, userId, metadata });
